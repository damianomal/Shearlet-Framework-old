...
for scale = 2%:3
    end_frame = 90;
    tic
    for i=[16 18] %1:numel(filenames) %define the files to be used.
...
for cluster_num =[1 2 3 6 7]
% for cluster_num =1
    
    file_num = 16:18; 
    
    %DEBUG to calculate things since it crashed after the first iteration
    if(cluster_num == 1)
        file_num = [17 18];
    end
...
%     end
    ...
    for i=file_num  % files to be clustered
        ...
        [cameras{camera_num(i)+1} '\' cluster '\' filenames{i} '_' cluster]
